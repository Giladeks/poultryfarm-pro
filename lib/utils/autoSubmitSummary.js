  ...
  if (pmAssignments.length > 0) {
    await prisma.notification.createMany({
      data: pmAssignments.map(a => ({
        ...
        message:     hasPending
          ? `Today's section summary has been auto-submitted but has ${agg.pendingEggVerifications + agg.pendingFeedVerifications + agg.pendingMortalityVerifications} pending verification(s). Please review.`
          : `Today's section summary has been auto-submitted. Eggs: ${agg.totalEggsCollected}, Feed: ${Number(agg.totalFeedKg).toFixed(1)} kg, Mortality: ${agg.totalMortality}.`,
        data: {
