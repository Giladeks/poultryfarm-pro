...
export async function POST(request) {
  ...
  try {
    ...
    const existing = await prisma.supplierInvoice.findFirst({
      where: { tenantId: user.tenantId, invoiceNumber: data.invoiceNumber },
    });
    if (existing) return NextResponse.json({ error: 'Invoice number already exists for this tenant' }, { status: 409 });
    ...
    await prisma.auditLog.create({
      data: {
        ...
        entityId:   invoice.id,
        changes:    { invoiceNumber: data.invoiceNumber, supplierId: data.supplierId, totalAmount: data.totalAmount },
      },
    });
