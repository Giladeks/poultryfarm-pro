...
// Fixed: FARM_OWNER → FARM_ADMIN (FARM_OWNER is not a valid role)
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:24 }}>
          ...
          {canCreate && (
            <div style={{ flexShrink:0 }}>
              <button onClick={() => setShowCreate(true)} className="btn btn-primary">+ New Flock Batch</button>
            </div>
          )}
        </div>
...
function FlockModal({ flock, onClose }) {
  ...
  return (
    ...
    >
      {/* Custom header with breed subtitle */}
...
// ── Create flock modal (portal) ───────────────────────────────────────────────
function CreateFlockModal({ apiFetch, onClose, onCreated }) {
  const [form, setForm] = useState({
    batchCode: '', birdType: 'LAYER', breed: '', penSectionId: '',
